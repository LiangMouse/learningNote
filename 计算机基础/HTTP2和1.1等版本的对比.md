...
1. **多路复用**：HTTP 2.0 支持多路复用，即在一个 TCP 连接上可以同时传输多个请求/响应。`1.1`的TCP连接只能串行进行数据的传输以及发送和处理请求，倘若一个请求返回慢，就会造成类似**堵车**的情况。HTTP2新特性多路复用避免了 HTTP 1.1 的**队头阻塞**问题(前边的请求处理慢，后边的都会堵塞)，提高了性能。
