...
- HTTP 的连接相对简单，TCP 三次握手便可进行 HTTP 传输，而 HTTPS 需要在 `TCP` 三次握手**后**进行 TLS 的握手才能进行加密报文的传输（HTTP3 进行了整合）
...
> 对称加密就是通信双方只有一把秘钥进行加密解密，而非对称加密有两个秘钥，一个公钥一个私钥。前者效率快，但秘钥泄露会导致加密内容泄露，后者则相反
