...
**重点**
foreach一定会改变数组内容吗？ 很多给出的答案中都是说`foreach`会改变数组内容，但这是错误的，要分开讨论
...
返回一个新数组（foreach不返回）,这个新数组由原数组元素依次执行回调的返回值组成，但不会改变原数组

参数是一个回调函数，其中有三个参数，第一个参数为当前遍历的元素，第二个参数为当前遍历的下标，第三个为用作遍历的数组本身。第一个为必选，其余都为可选
**map和foreach的区别** 
- map会返回一个新数组，而foreach不会
- map可以链式调用，可以对返回新数组继续操作，而foreach不能
- foreach可以对包含引用数据的数组进行修改，而map不能
- foreach使用return可以中断本次循环(类似continue)，而map不能
...
arr.slice(l,r)也就是**截取**这个数组第 l 下标到 r-1(包括) 下标的内容，也就是左闭右开(和字符串的slice相同)
