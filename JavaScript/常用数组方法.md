...
- 求一个数组中所有元素的最大值 `Math.max(...arr)`

## 常见数组 API 以及使用

我自己整理的一些好用的 api，但感觉比较好的方法是看到什么[mdn](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Array/at)上不认识的就看一下，有个大概印象，让我们知道 JS 数组原型上已经有这样的方法能做这样的事情，然后再去大模型或者看文档具体使用方法，参数列表就 Ok 了
...
可以理解为如果参数是数组，就进行解构然后拼起来，如果是原始类型数据就直接拼起来
...
与上相同，findIndex() 方法接受测试函数作为参数，符合满足测试函数的第一个元素的索引。若没有找到对应元素则返回 -1。
...
> 此外还有 findLast()和 findLastIndex()功能与上边类似，分别对应，区别在于这两个 api 为反向迭代
